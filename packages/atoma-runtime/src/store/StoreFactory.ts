...
export class StoreFactory {
    ...
    build = <T extends Entity = Entity>(storeName: string): StoreFactoryResult<T> => {
        ...
        const getAll: StoreEngineApi<T>['getAll'] = (filter, cacheFilter, options) => this.runtime.read.getAll(handle, filter, cacheFilter, options)
        const getMany: StoreEngineApi<T>['getMany'] = (ids, cache, options) => this.runtime.read.getMany(handle, ids, cache, options)
        const getOne: StoreEngineApi<T>['getOne'] = (id, options) => this.runtime.read.getOne(handle, id, options)
