...
import type { CoreRuntime, Entity, StoreDispatchEvent } from 'atoma-core/internal'
...
function applyUpsertsWriteback<T extends Entity>(args: {
    ...
    for (const item of args.upserts) {
        ...
        const preserved = existing ? StoreWriteUtils.preserveReferenceShallow(existing, item) : item
...
function applyCreatedWriteback<T extends Entity>(args: {
    ...
    for (const op of args.operations) {
        ...
        const value = StoreWriteUtils.preserveReferenceShallow(existing, serverItem)
...
function applyVersionWriteback<T extends Entity>(args: {
    ...
    for (const [key, version] of versionByKey.entries()) {
        ...
        next.set(key, StoreWriteUtils.preserveReferenceShallow(curVersioned, merged))
        changedIds.add(key)
    }
