import type { SortRule } from '@atoma-js/types/core'
...
export function compareBy<T>(rules: SortRule[]): (a: T, b: T) => number {
    return (a, b) => {
        for (const rule of rules) {
            const av = read(a, rule.field)
            const bv = read(b, rule.field)
            if (av === bv) continue
...
export function getSortValues<T>(item: T, rules: SortRule[]): unknown[] {
    return rules.map(rule => read(item, rule.field))
}
